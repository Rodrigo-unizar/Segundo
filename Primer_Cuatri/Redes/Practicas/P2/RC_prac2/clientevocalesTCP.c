...
int initsocket(struct addrinfo *servinfo, char f_verbose)
{
    ...
    {   // bucle que recorre la lista de direcciones
        ...
        sock = socket(AF_INET,SOCK_STREAM , 0 );//dominio tipo protocolo
        if (sock < 0)
        {
...
int main(int argc, char * argv[])
{
    ...
    {
        perror("Error al cerrar el socket para escritura (shutdown)");
        exit(1);
    }
    ...
    recvbytes = recv(sock,&num , sizeof num, 0); //socket, mensaje, long msj, flags
