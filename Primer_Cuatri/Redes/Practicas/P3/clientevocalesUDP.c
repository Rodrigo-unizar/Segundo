...
int initsocket(struct addrinfo *servinfo, char f_verbose)
{
    ...
    {   // bucle que recorre la lista de direcciones
        ...
        sock = socket(AF_INET,SOCK_DGRAM , 0 );//dominio tipo protocolo
        if (sock < 0)
        {
...
int main(int argc, char * argv[])
{
    ...
    // como ya se ha liberado e se bloque de memoria,
    ...
    while ((len = read(0, msg, MAX_BUFF_SIZE)) > 0 && msg[0] != fin)
    {
        ...
        {
            perror("Error de escritura en el socket (send)");
            exit(1);
        }
        else
        {
    ...
    msg[0] = fin;
    sendto(sock, msg, 1, 0, servinfo->ai_addr, servinfo->ai_addrlen);
    ...
    recvbytes = recvfrom(sock,&num , sizeof num, 0, servinfo->ai_addr, &servinfo->ai_addrlen); //socket, mensaje, long msj, flags
    ...
    if (close(sock) < 0)
    {
        perror("Error al cerrar el socket (close)");
        exit(1);
    }
    else
    {
